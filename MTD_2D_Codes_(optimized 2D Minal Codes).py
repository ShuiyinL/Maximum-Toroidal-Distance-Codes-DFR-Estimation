...
def main():
    print('Computing DFR of 2D-GTD Code')
...
if __name__ == "__main__":
    ...
    Q = 3329
    N = 2   # GTC code dimension (or block length)
    eta = 2
    k = 4
    ...
    # 2D-GTD Codebook (4 codewords from optimized 2D-MTD Code)
    p=2*56; 
    D2_4_points = {
    0: (0, 0),
    1: (14, 56),
    ...
    for j in range(size_codebook):
        ...
        for i in code_copy:
            diff = [
                mod_sym(vi - ci, p)
                for vi, ci in zip(D2_4_points[j], D2_4_points[i])
            ]
